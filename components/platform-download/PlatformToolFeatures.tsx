...
const statIcons = [Zap, Shield, MonitorPlay, Globe, Star, Copy];
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        >
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-foreground mt-3 mb-4 font-heading">
            {t("toolFeaturesHeading", { name: config.name })}
          </h2>
          <p className="text-muted-foreground text-sm max-w-xl mx-auto font-sans">
            {t("toolFeaturesSubheading", { name: config.name })}
          </p>
        </motion.div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {(t.raw("toolFeaturesStats") as { title: string; value: string; desc: string }[]).map((stat, i) => {
                ...
                <div className="flex items-start gap-3.5">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-baseline gap-2 flex-wrap">
                      <h3 className="text-sm font-bold text-foreground font-heading">{stat.title}</h3>
                      <span
