            ...
            <>
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1 lg:gap-1.5 xl:gap-2 pl-0.5 lg:pl-1 pr-1 lg:pr-1.5 xl:pr-3 py-0.5 lg:py-1 rounded-full border border-border/60 hover:border-border bg-white shadow-sm hover:shadow-md transition-all shrink-0"
                  aria-label="Account menu"
                >
                  {user?.avatar_url ? (
                    <img src={user.avatar_url} alt={user.name || "Account"} className="w-6 h-6 lg:w-6 lg:h-6 xl:w-7 xl:h-7 rounded-full object-cover border border-border/40" />
                  ) : (
                    <span className="w-6 h-6 lg:w-6 lg:h-6 xl:w-7 xl:h-7 rounded-full bg-[#0d1f26] dark:bg-white text-white dark:text-[#0d1f26] flex items-center justify-center text-xs font-bold shadow-sm">
                      {(user?.name || user?.email || "U")[0]?.toUpperCase()}
                    </span>
                  )}
                  <span className="hidden xl:block text-sm font-semibold text-foreground max-w-[110px] lg:max-w-[100px] xl:max-w-[120px] truncate font-sans">
                    {user?.name || user?.email}
                  </span>
                  <ChevronDown className={`w-3 h-3 lg:w-3 lg:h-3 xl:w-3.5 xl:h-3.5 text-muted-foreground transition-transform ${accountOpen ? "rotate-180" : ""}`} />
                </button>
                <AnimatePresence>
                  {accountOpen && (
                    <motion.div
