...
export default async function VideoDownloaderPage({ params }: Props) {
  ...
  const st = await getTranslations({ locale, namespace: "PlatformShared" });
  ...
  // Localized tool label ("Video Downloader" / "Descargador de video" / …) so
  // structured data matches the language of the page.
  const typeVideo = (() => {
    try {
      return st("typeVideo");
    } catch {
    ...
    "@graph": [
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", position: 2, name: `${config.name} ${typeVideo}`, item: `https://www.downforge.me/${locale}/video-downloader/${config.slug}` },
        ],
      },
      {
        "@type": "WebApplication",
        "@id": `https://www.downforge.me/${locale}/video-downloader/${config.slug}#webapp`,
        name: `${config.name} ${typeVideo} — DownForge`,
        url: `https://www.downforge.me/${locale}/video-downloader/${config.slug}`,
