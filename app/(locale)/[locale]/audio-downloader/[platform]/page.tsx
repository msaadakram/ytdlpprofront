...
export default async function AudioDownloaderPage({ params }: Props) {
  ...
  // Type-specific templated FAQs — fully translated in every locale (the
  // per-platform faqsAudio arrays are English-only outside EN).
  const dl = await getTranslations({ locale, namespace: "DownloadOnly" });
  const et = await getTranslations({ locale, namespace: "ExploreTools" });
  const faqs =
    (await getTemplatedFaqs(locale, "audio", config.name)) ??
    (t.raw("faqsAudio") as { q: string; a: string }[]);
    ...
    "@graph": [
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", position: 2, name: `${config.name} ${et("tools.audio.label")}`, item: `https://www.downforge.me/${locale}/audio-downloader/${config.slug}` },
        ],
      },
      {
        "@type": "WebApplication",
      ...
      },
      {
        ...
        name: dl("howToAudioName", { platform: config.name }),
        description: dl("howToAudioDesc", { platform: config.name }),
        totalTime: "PT30S",
  ...
  return (
    <>
      ...
      <main>
        ...
        <RelatedLinks links={relatedLinksFor("audio-downloader", platform)} />
        <ExploreOtherTools platform={platform} currentTool="audio" />
      </main>
      <Footer />
    </>
  );
