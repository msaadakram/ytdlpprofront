...
export default async function ThumbnailDownloaderPage({ params }: Props) {
  ...
  const faqs =
    (await getTemplatedFaqs(locale, "thumbnail", config.name)) ??
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: `${config.name} ${et("tools.thumbnail.label")}`, item: `https://www.downforge.me/${locale}/thumbnail-downloader/${config.slug}` },
        ],
      },
      {
        "@type": "WebApplication",
