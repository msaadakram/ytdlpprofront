...
export default async function TranscriptDownloaderPage({ params }: Props) {
  ...
  // Type-specific templated FAQs (translated in every locale) — the video-intent
  // `faqs` array previously leaked audio questions onto this page's JSON-LD.
  const et = await getTranslations({ locale, namespace: "ExploreTools" });
  const faqs =
    (await getTemplatedFaqs(locale, "transcript", config.name)) ??
    (t.raw("faqs") as { q: string; a: string }[]);
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: `${config.name} ${et("tools.transcript.label")}`, item: `https://www.downforge.me/${locale}/transcript-downloader/${config.slug}` },
        ],
      },
      {
        "@type": "WebApplication",
