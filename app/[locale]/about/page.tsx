...
export default async function AboutPage({ params }: Props) {
  ...
  try {
    ...
    if (Array.isArray(rawTl)) rawTl.forEach((item, i) => { if (timeline[i]) timeline[i] = item as typeof timeline[0]; });
  } catch {}
  ...
  try {
    ...
    if (Array.isArray(rawVals)) rawVals.forEach((v, i) => { if (values[i]) values[i] = v as typeof values[0]; });
  } catch {}
