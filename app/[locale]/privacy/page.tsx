      ...
      <main className="pt-16 sm:pt-20">
        ...
        <section className="bg-[#0d1f26] text-white relative overflow-hidden">
          ...
          <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/10 px-3 py-1.5 mb-4 backdrop-blur">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-[2.75rem] font-black tracking-tight font-heading">{t("title")}</h1>
            <p className="mt-3 inline-flex items-center gap-2 text-xs font-medium text-white/60 bg-white/5 border border-white/10 rounded-full px-3 py-1.5">
              <Clock className="w-3 h-3" /> {t("lastUpdated")}
            </p>
            <p className="mt-4 text-sm sm:text-base leading-relaxed text-white/60 max-w-2xl font-sans">
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:py-12">
          <div className="grid lg:grid-cols-[280px_1fr] gap-6 lg:gap-10 items-start">
            ...
            <div className="min-w-0">
              <div className="rounded-[1.75rem] bg-white dark:bg-white/[0.04] border border-border/60 dark:border-white/10 shadow-sm overflow-hidden">
                  ...
                  <div className="space-y-8">
                    {sections.map((s) => (
                      <section key={s.id} id={s.id} className="scroll-mt-24">
                        <h2 className="flex items-start gap-3 text-lg sm:text-xl font-bold text-foreground font-heading">
                          <span className="w-9 h-9 rounded-xl bg-[#0d1f26] dark:bg-white text-white dark:text-[#0d1f26] flex items-center justify-center shrink-0 shadow-sm mt-0.5">
                            <s.icon className="w-4 h-4" />
                          </span>
                          <span>{s.title}</span>
                        </h2>
                        <p className="mt-3 ml-12 text-sm sm:text-[15px] leading-relaxed text-muted-foreground font-sans">{s.body}</p>
                      </section>
                    ))}
                  </div>
                  <div className="mt-10 flex flex-wrap gap-3 border-t border-border/50 pt-6">
                    <Link href="/terms" className="inline-flex items-center gap-2 text-sm font-semibold bg-[#0d1f26] dark:bg-white text-white dark:text-[#0d1f26] px-5 py-2.5 rounded-full hover:bg-[#1a3545] dark:hover:bg-slate-100 transition-colors">
                      Terms of Service <ArrowRight className="w-4 h-4" />
                    </Link>
                    <Link href="/contact" className="inline-flex items-center gap-2 text-sm font-semibold bg-white dark:bg-white/10 border border-border dark:border-white/10 text-foreground px-5 py-2.5 rounded-full hover:bg-muted/50 transition-colors">
                      Contact support
                    </Link>
                  </div>
                </div>
